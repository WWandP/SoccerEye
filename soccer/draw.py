...
#id暂时作为是否开启
    ...
    def draw_detection(
        ...
        if "id" in detection.data and id is True:
            ...
            teamname=detection.data["classification"]
...
class PathPoint:
    ...
    def from_abs_bbox(
        ...
        center = PathPoint.get_center_from_bounding_box(abs_point)
...
class AbsolutePath:
    ...
    def add_new_point(
        self, points: np, color: tuple = (255, 255, 255)
    ) -> None:
        """
        ...
        self.past_points.append(points)

        self.color_by_index.append(color)
    ...
    def draw(
        ...
        if not ball_detections:
            if self.past_points:
                ...
                self.color_by_index.pop(0)
            else:
                return img # 如果past_points为空，则不再执行后面的代码
        else:
            coords = [ball_detections[0].points[0], ball_detections[0].points[1]]
            #应要求足球轨迹改为黄色
            self.add_new_point(points=coords, color=(30,255,255))
        max_points = 20  # 你可以根据需要调整这个值
        if len(self.past_points) < 2:
            return img
        if len(self.past_points) > max_points:
            self.past_points = self.past_points[-max_points:]
            self.color_by_index=self.color_by_index[-max_points:]
