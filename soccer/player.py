...
from soccer.draw import Draw, PathPoint, AbsolutePath
from soccer.team import Team
import cv2
from PIL import Image


def draw_ellipse(image, xywh: list, color: tuple, thickness: int = 2) -> Image:
    x, y, width = xywh[:3]
    width = int(width)
    cv2.ellipse(
        image,
...
class Player:
    ...
    def distance_to_ball(self, ball: Ball) -> float:
        ...
        if not self.detection  or ball.center is None:
            return None
    ...
    def draw(
        ...
        if self.team is not None:
            ...
            if self.detection.data["classification"] == "Referee":
                return frame
            # arrow
            rectangle = Draw.draw_pointer(self.detection, frame, self.detection.data["color"])
            # ellipse
            x_center = (self.detection.points[1][0] + self.detection.points[0][0]) // 2
            y_center = self.detection.points[1][1]
            # weight = self.detection.points[1][0]-self.detection.points[0][0]
            weight = 45
            xyw = [x_center, y_center, weight]
            frame = np.array(rectangle)
            frame = draw_ellipse(image=frame, xywh=xyw, color=self.detection.data["color"])
        return frame
        # original
        # return Draw.draw_detection(self.detection, rectangle, confidence=confidence, id=id)
    ...
    def draw_players(
        ...
        #     if "id" in player.detection.data and id is True:
        #         if player.detection.data["id"] == 8:
        #             x_bottem_center=(player.detection.points[0][0]+player.detection.points[1][0])/2
        #             ybottem_center=player.detection.points[1][1]
        #             player.player_croods_id1.append((x_bottem_center,ybottem_center))
        # print(player.player_croods_id1)
        return frame
