...
from perspective_transform.camera_pose_estimation.projector import (
    draw_pitch_lines,
    project_to_screen,
)

from perspective_transform.pitch_tracker.common import intersect
from perspective_transform.pitch_tracker.key_points import (
    corner_front_right_world,
    corner_front_left_world,
    corner_back_right_world,
    corner_back_left_world,
)
from perspective_transform.pitch_tracker.main import find_key_points
...
def can_add_coordinate(coordinate, recorded_coordinates):
    """
    Check if a coordinate can be added to the list.
    """
    # 如果列表中已存放的坐标数量已经达到上限，不可再添加
    if len(recorded_coordinates) == 0:
        return True

    # 遍历已存放的坐标，如果当前坐标与已存放的坐标不同行同列，返回True
    for recorded_coordinate in recorded_coordinates:
        if coordinate[0] != recorded_coordinate[0] and coordinate[1] != recorded_coordinate[1]:
            return True

    # 如果与所有已存放的坐标都不同行同列，则返回False
    return False
    ...
    for i in range(height):
        ...
        for j in range(1668):
            ...
            if 0 <= u < width and 0 <= v < height:
                perspective_src.append([u, v])
                perspective_dst.append([j + offset_x, i])
                break
        for j in reversed(range(1668)):
            x_world = corner_front_left_world[0] + j * width_soccer_field / pixels_width
            proj = project_to_screen(
                K, to_device_from_world, np.array([x_world, 0, z_world])
            )
    ...
    for i in reversed(range(height)):
        ...
        for j in reversed(range(1668)):
            ...
            if 0 <= u < width and 0 <= v < height:
                ...
                perspective_dst.append([j + offset_x, i])
                break
        if len(perspective_src) >= 4:
            break

    correspondences_src = np.array(perspective_src).astype(np.float32)
    correspondences_dst = np.array(perspective_dst).astype(np.float32)
    M = cv2.getPerspectiveTransform(correspondences_src, correspondences_dst)
    unskewd = cv2.warpPerspective(img, M, (width, height))
    return unskewd
def homography_top_view(K, to_device_from_world, img):
    """
    Display top view of the image, by unprojecting every pixels of the image
    to its correspondance in the world, assuming that altitude is 0
    """
