...
# def draw_boxes(img, bbox, colors, identities=None, offset=(0, 0)):
def draw_boxes(img, players, offset=(0, 0)):
    for player in players:
        x1, y1, x2, y2 = [val for sublist in player.detection["bbox"] for val in sublist]
        # box text and bar
        id = int(player.detection["id"]) if player.detection["id"] is not None else 0
        # color = compute_color_for_labels(id)
        color = player.team.color if player.team.color is not None else (0, 0, 0)
        label = '{}{:d}'.format("", id)
